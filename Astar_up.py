...
class Node:
    ...
    def set_h(self, end_Node):
        self.h = 0
        hold_h = Manhaton.get(str(self.nodeChess))
        if hold_h:
            self.h = int(hold_h)
        else:
            for i in range(4):
                for j in range(4):
                    if self.nodeChess[i][j] != 0 and self.nodeChess[i][j] != (
                            i * 4 + j + 1):
                        ii = (self.nodeChess[i][j] - 1) // 4
                        jj = (self.nodeChess[i][j] - 1) % 4
                        self.h += (abs(ii - i) + abs(jj - j))
            Manhaton[str(self.nodeChess)] = int(self.h)
...
class A:
    ...
    def get_Nodeinclose(self, speci_node):
        return self.closelist.get(str(speci_node.nodeChess))
    ...
    def Astar_p(self):
        ...
        while True and not self.openlist.queue_empty():
            ...
            self.closelist[str(self.curNode.nodeChess)] = self.curNode
...
if __name__ == "__main__":
    ...
    if a.result_path:
        ...
        for i in thefinal_path:
            print(i)
        with open("C:/Users/17422/Desktop/holddata.txt", "w") as f:
            for i in thefinal_path:
                f.write(str(i))
                f.write("\n")
    else:
